...
function useMotionControls(motionProperties, variants = {}, { motionValues, push, stop } = useMotionTransitions()) {
  ...
  const set3 = (variant) => {
    const variantData = isObject2(variant) ? variant : getVariantFromKey(variant);
...
function resolveVariants(node, variantsRef) {
  ...
  if (target) {
    if (target.variants && isObject2(target.variants)) {
      variantsRef.value = {
        ...variantsRef.value,
        ...target.variants
      };
    ...
    directivePropsKeys.forEach((key) => {
      ...
      if (key === "visible-once")
        key = "visibleOnce";
      if (target && target[key] && isObject2(target[key]))
